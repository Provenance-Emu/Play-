...
void CGSH_Vulkan_Libretro::PresentBackbuffer(uint32_t sync_index)
{
    ...
    vk_image.create_info.format = VK_FORMAT_B8G8R8A8_SRGB;  // Restore more accurate BGRA8888 format
...
void CGSH_Vulkan_Libretro::FlipImpl(const DISPLAY_INFO& displayInfo)
{
    ...
    // BACK BUFFER FIX: Copy image BEFORE base flip to capture current frame
    // The base flip might swap buffers, so we need to copy before that happens
    ...
    // BACK BUFFER FIX: Copy image BEFORE flip to get the correct current frame
    // This prevents showing the "next below" frame that the user reported
    if (m_vk_iface && m_vk_iface->handle) {
        uint32_t sync_index = m_vk_iface->get_sync_index(m_vk_iface->handle);
        VkImage libretro_image = GetOrCreateLibretroImage(sync_index);
        if (libretro_image != VK_NULL_HANDLE) {
            g_log_cb(RETRO_LOG_DEBUG, "[libretro DEBUG] Copying frame BEFORE flip (sync_index=%u)\n", sync_index);
            CopyPlayDrawImageToLibretro(libretro_image, sync_index);
            // FLICKER FIX: Pass the same sync_index to PresentBackbuffer
            PresentBackbuffer(sync_index);
        } else {
            g_log_cb(RETRO_LOG_ERROR, "[libretro ERROR] Failed to create libretro image for sync_index %u\n", sync_index);
        }
    } else {
        g_log_cb(RETRO_LOG_ERROR, "[libretro ERROR] Vulkan interface not available\n");
    }

    // Call base CGSHandler::FlipImpl AFTER copying to avoid buffer swap issues
    // This ensures we captured the current frame before any buffer management
...
VkImage CGSH_Vulkan_Libretro::GetOrCreateLibretroImage(uint32_t sync_index)
{
    ...
    image_info.format = VK_FORMAT_B8G8R8A8_SRGB;  // Restore more accurate BGRA8888 format
...
void CGSH_Vulkan_Libretro::CopyPlayDrawImageToLibretro(VkImage dst_image, uint32_t sync_index)
{
    ...
    if (drawSystem) {
        // MOBILE DRAW FIX: Cast to CDrawMobile and use GetDrawImage() method
        auto drawMobile = std::dynamic_pointer_cast<GSH_Vulkan::CDrawMobile>(drawSystem);
        if (drawMobile) {
            RETRO_LOG(RETRO_LOG_INFO, "CDrawMobile cast result: %p\n", drawMobile.get());
            
            // Get the draw image using the new public getter
            const Framework::Vulkan::CImage& drawImage = drawMobile->GetDrawImage();
            src_image = static_cast<VkImage>(drawImage);  // Use operator VkImage()
            
            if (src_image != VK_NULL_HANDLE) {
                RETRO_LOG(RETRO_LOG_INFO, "SUCCESS: Got CDrawMobile draw image: %p\n", (void*)src_image);
                has_source_image = true;
            } else {
                RETRO_LOG(RETRO_LOG_ERROR, "ERROR: CDrawMobile draw image is VK_NULL_HANDLE\n");
                has_source_image = false;
            }
        } else {
            // Fallback: Try CDrawDesktop for compatibility
            auto drawDesktop = std::dynamic_pointer_cast<GSH_Vulkan::CDrawDesktop>(drawSystem);
            if (drawDesktop) {
                RETRO_LOG(RETRO_LOG_INFO, "Using CDrawDesktop fallback\n");
                const Framework::Vulkan::CImage& drawImage = drawDesktop->GetDrawImage();
                src_image = static_cast<VkImage>(drawImage);  // Use operator VkImage()
                has_source_image = (src_image != VK_NULL_HANDLE);
            } else {
                RETRO_LOG(RETRO_LOG_INFO, "Draw system is neither CDrawMobile nor CDrawDesktop\n");
            }
        }
    } else {
